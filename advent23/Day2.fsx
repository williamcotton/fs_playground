...
let validGame maxColor =
    let (maxRed, maxGreen, maxBlue) = maxColor
...
let a =
    ...
    |> Array.fold (fun (gameNumber, sumOfIdsOfGames) game ->
        ...
        | maxColor when validGame maxColor -> (gameNumber + 1, sumOfIdsOfGames + gameNumber)
        | _ -> (gameNumber + 1, sumOfIdsOfGames)
      ) (1, 0)
    |> snd
